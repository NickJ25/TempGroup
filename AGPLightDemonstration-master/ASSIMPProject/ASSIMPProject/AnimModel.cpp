...
void AnimModel::loadModel(const string& path)
{
	...
	searchNode(scene->mRootNode);
}

void AnimModel::searchNode(aiNode* node)
{
	if(node->mName.data == string("Armature_Upper_Left_Arm"))
	{
		cout << "found arm" << endl;
		armNode = node;
	}
	else {
		cout << "node is called : " << node->mName.data << " great sucess." << endl;
		// After we've processed all of the meshes (if any) we then recursively process each of the children nodes
		for (uint i = 0; i < node->mNumChildren; i++)
		{
			searchNode(node->mChildren[i]);
		}
	}
...
void AnimModel::readNodeHierarchy(float p_animation_time, const aiNode* p_node, const aiMatrix4x4 parent_transform)
{
	...
	if (node_anim)
	{
		...
		if (string(node_anim->mNodeName.data) == string("Armature_Upper_Left_Arm"))
		{
