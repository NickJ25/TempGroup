...
void SkyBox::init(const char* folder)
{
	...
	const char *lezTexFiles[6] = {
		"res/FogMap/back.bmp", "res/FogMap/front.bmp", "res/FogMap/right.bmp", "res/FogMap/left.bmp", "res/FogMap/top.bmp", "res/FogMap/bottom.bmp"
	};

	cube_texture_id = loadCubeMap(lezTexFiles, &cube_texture_id);
...
void SkyBox::draw()
{
	...
	glBindVertexArray(VAO_skybox);
	glUniformMatrix4fv(glGetUniformLocation(skyBoxShader->Program, "VP"), 1, GL_FALSE, glm::value_ptr(VP_matrix));
...
GLuint SkyBox::createCubeTexture(const char* folder)
{
	GLuint text_id;
